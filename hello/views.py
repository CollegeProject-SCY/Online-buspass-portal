...
def verification(request,item_id=None):
    if request.method =='POST':
        ...
        try:
            ...
            my_time=datetime.fromisoformat(otp_time)
            
            future_time = my_time + timedelta(minutes=n)
...
def callback(request):
    if request.method == 'POST':
        try:        
            ...
            data=applicants.objects.get(pass_id=pass_id)
            photo=str(data.passport_size_image)
            link='media/'+photo
            Context={
                'data':data,
            ...
            if status=='TXN_SUCCESS':
                ...
                if Transaction.objects.filter(username=username).exists():
                    ...
                    bus_pass(request,Context,pass_id,expire_date)
                    obj.save()
